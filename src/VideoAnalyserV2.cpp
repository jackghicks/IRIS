...
namespace iris
{
	VideoAnalyserV2::VideoAnalyserV2(Configuration* configuration, irisProgressCallback* callback)
	{
	...
	VideoAnalyserV2::~VideoAnalyserV2()
	{
	}

	void VideoAnalyserV2::Init(const std::string& videoName, bool flagJson)
	{
	...
	void VideoAnalyserV2::AnalyseVideo(bool flagJson, const char* sourceVideo)
	{
		...
		if (VideoIsOpen(sourceVideo, video, videoName.c_str()))
		{
			...
			(*m_callback)(nullptr, FrameData().CsvColumns().c_str());
			...
			while (!frame.empty())
			{
				...
				(*m_callback)(nullptr, data.ToCSV().c_str());
	...
	void VideoAnalyserV2::AnalyseFrame(cv::Mat& frame, unsigned int& frameIndex, FrameData& data)
	{
	...
	void VideoAnalyserV2::SetOptimalCvThreads(cv::Size size)
	{
		int num_threads = 1;
	...
	bool VideoAnalyserV2::VideoIsOpen(const char* sourceVideo, cv::VideoCapture& video, const char* videoName)
	{
		if (video.isOpened())
		{
	...
	void VideoAnalyserV2::UpdateProgress(unsigned int& numFrames, const unsigned long& totalFrames, unsigned int& lastPercentage)
	{
	...
	int VideoAnalyserV2::GetVideoFps(const char* filePath)
	{
	...
	void VideoAnalyserV2::SerializeResults(const Result& result, const FrameDataJson& lineGraphData, const FrameDataJson& nonPassData)
	{
